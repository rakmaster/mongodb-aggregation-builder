...
import { VExpansionPanel, VExpansionPanelTitle, VExpansionPanelText, VRow, VCol, VChip, VSelect, VBtnGroup, VBtn, VAlert } from 'vuetify/components'
...
const stagePreview = computed(() => {
  try {
    const parsed = JSON.parse(stageJson.value)
...
watch(() => props.stage, (newStage) => {
  const newJson = JSON.stringify(newStage, null, 2)
  if (stageJson.value !== newJson) {
    selectedStageType.value = Object.keys(newStage)[0] || ''
    stageJson.value = newJson
    validationErrors.value = []
  }
}, { immediate: true })
</script>
